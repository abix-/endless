...
def rpc(method, params=None):
    """Send any JSON-RPC request to the game server. Works with any BRP method."""
...
def summary(town=None):
    ...
    return rpc("endless/summary", params)
...
def set_personality(town, personality):
    ...
    return rpc("endless/ai_manager", {"town": town, "active": True, "personality": personality})
...
def set_policy(town, **kwargs):
    ...
    return rpc("endless/policy", {"town": town, **kwargs})

def buy_upgrade(town, upgrade_idx):
    """Queue an upgrade purchase by index."""
    return rpc("endless/upgrade", {"town": town, "upgrade_idx": upgrade_idx})

def target_squad(squad, x, y):
    """Send a squad to a position."""
    return rpc("endless/squad_target", {"squad": squad, "x": x, "y": y})
...
def build(town, kind, row, col):
    ...
    return rpc("endless/build", {"town": town, "kind": kind, "row": row, "col": col})
...
def set_time(paused=None, time_scale=None):
    ...
    return rpc("endless/time", params)
