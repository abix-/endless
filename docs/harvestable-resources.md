...
All resource harvesting uses one unified system: WorksiteDef + ProductionState + ActivityKind::Work. There are no per-resource activity kinds or separate harvest loops.

## Resource types

| Resource | Source | Node/Building kind | Storage | Harvester building | Worker job | Growth | Destruction |
|----------|--------|-------------------|---------|-------------------|------------|--------|-------------|
...
## Unified harvest architecture

Every harvestable entity follows the same pipeline:

```
BuildingDef.worksite: WorksiteDef    -- registry config (harvest_item, one_shot, etc.)
ProductionState                      -- ECS component (ready flag, growth progress)
growth_system                        -- controller: advances progress -> sets ready
decision_system (ActivityKind::Work) -- controller: NPC claims worksite, harvests when ready
...
pub struct WorksiteDef {
    ...
    pub one_shot: bool,  // true = destroy entity after harvest (TreeNode, RockNode)
}
```
...
- `TreeNode | RockNode`: instant ready (always harvestable when awake)
...
3. **Work(Active) -> harvest**: when `ProductionState.ready`, harvest via `ProductionState::harvest_with_mode()`, add yield to `carried_loot` via `WorksiteDef.harvest_item` match
4. **Post-harvest**: if `WorksiteDef.one_shot`, destroy the worksite entity. Release worksite claim.
...
No separate `ActivityKind::Chop` or `ActivityKind::Quarry`. All harvesters use `ActivityKind::Work`.

### carried_loot writeback

The decision_system writeback block must persist ALL carried_loot fields (food, gold, wood, stone), not just a subset. The generic worksite harvest path writes to the correct field via `WorksiteDef.harvest_item` -> `ResourceKind` match.

## CRD compliance

All types follow the Def -> Instance -> Controller pattern (see `docs/k8s.md`):

- **TreeNode/RockNode**: `BuildingDef` entries in `BUILDING_REGISTRY` with `worksite: Some(WorksiteDef { one_shot: true, ... })`. Spawned via `place_building()`, destroyed on harvest.
...
Adding a new resource type requires: 1 BuildingKind + 1 registry entry + 1 ResourceKind variant + 1 Store component + 1 Job + 1 NPC registry entry + growth_system match arm. No new activity kinds, no new harvest loops, no new writeback paths.
...
- One harvest destroys the node permanently (via `WorksiteDef.one_shot: true`)
