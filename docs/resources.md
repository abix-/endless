...
| BuildingEntityMap | bidirectional `HashMap<(BuildingKind, usize), usize>` + `slot_to_entity: HashMap<usize, Entity>` | Maps buildings ↔ GPU slots ↔ Bevy entities; replaces both old `BuildingSlotMap` and building entries in `NpcEntityMap`; buildings are NOT in `NpcEntityMap` — they have their own identity infrastructure; methods: `insert`/`set_entity`/`insert_full`/`get_slot`/`get_building`/`is_building`/`get_entity`/`get_entity_by_building`/`remove_by_building`/`clear` |
