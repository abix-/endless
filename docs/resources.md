...
| SpawnerState | `Vec<SpawnerEntry>` — building_kind (SPAWNER_FARMER/ARCHER/TENT/MINER), town_idx, position, npc_slot, respawn_timer. `is_population_spawner()` helper checks all 4 types. | game_startup, build_menu (push on build), spawner_respawn_system | spawner_respawn_system, game_hud (counts), ai_player (reserve calc) |
