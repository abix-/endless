...
| BuildingEntityMap | `BuildingInstance` storage + 256px spatial grid + by_kind/by_entity/by_grid_cell indexes | Sole source of truth for all building instance data (no WorldData.buildings); stores `BuildingInstance` (kind, position, town_idx, slot, entity, faction, patrol_order, assigned_mine, manual_mine, wall_level, npc_slot, respawn_timer); methods: `add_instance`/`remove_instance`/`get_instance[_mut]`/`find_by_position`/`iter_kind`/`iter_kind_for_town`/`count_for_town`/`building_counts`/`gold_mine_index`/`for_each_nearby` (spatial)/`iter_instances`/`iter_instances_mut`; GPU slot maps: `insert`/`get_slot`/`get_building`/`get_entity`/`get_entity_by_building` |
