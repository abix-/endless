...
rust/
  ...
  src/ui/
    ...
    game_hud.rs         # Top bar (food + gold + FPS), jukebox overlay (track picker + pause/skip/loop/speed), floating inspector with combat stats/equipment/status/carried loot + clickable faction link (bottom-left) + combat log with faction filter dropdown + clickable ">>" camera-pan on location entries (bottom-right), registry-driven spawner inspector (def.spawner + npc_def for labels, linked NPC state/squad/patrol/position via NpcStateQuery), mine assignment UI (click-to-assign), target overlay, squad overlay, expanded Copy Debug Info (NPC: XP/traits/stats/equipment/squad/gold/miner details; Building: faction/per-type details)
