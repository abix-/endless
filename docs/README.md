...
rust/
  ...
  src/systems/
    ...
    ai_player.rs        # AI decision system with personalities (Aggressive/Balanced/Economic), weighted random scoring, AiBuildRes SystemParam bundle, AiTownSnapshot caching with smart slot scoring (balanced_farm_ray_score, balanced_house_side_score), MineAnalysis single-pass (analyze_mines), try_build_scored/try_build_inner unified build helpers, wilderness waypoint placement near uncovered mines, sync_patrol_perimeter_system (dirty-flag-gated waypoint pruning with full destroy_building teardown)
