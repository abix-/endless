...
rust/
  ...
  src/ui/
    mod.rs              # register_ui(), game startup (+ policy load), cleanup, pause menu (+ debug settings + UI scale + audio volume + AI decision logging), escape/time controls (Playing + Running states), keyboard toggles (Q=squads, H=help), build ghost preview (Road drag-line with ghost trail), slot indicators, process_destroy_system, apply_ui_scale
