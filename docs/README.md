...
Bevy ECS (lib.rs build_app)
    ...
    │   ├─ Pause menu (ESC): Resume, tabbed Settings panel (PauseSettingsTab: Interface/Camera/Audio/Logs/Debug/SaveGame/LoadGame) with scrollable content, game quicksave/quickload + manual named save/load (named_save_path), separate settings-file save/reload/reset controls, Exit to Main Menu — available in both Playing and Running (test scenes)
...
rust/
  ...
  src/ui/
    mod.rs              # register_ui(), game startup (+ policy load), cleanup, pause menu (tabbed PauseSettingsTab: Interface/Camera/Audio/Logs/Debug/SaveGame/LoadGame with save/load/reload/reset actions + quicksave/quickload + named save/load), escape/time controls (Playing + Running states), keyboard toggles (Q=squads, H=help), build ghost preview (Road drag-line with ghost trail), slot indicators, process_destroy_system (resolves exact building slot by kind+town+grid before lethal DamageMsg), apply_ui_scale, apply_interface_text_size (global egui text sizing from UserSettings), test scene systems (bottom panel + target overlay + pause menu in Running state)
