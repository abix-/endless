...
rust/
  ...
  src/render.rs         # 2D camera, texture atlases (SpriteAssets with registry-driven external_textures Vec), TilemapChunk spawning (terrain only), TerrainChunk sync, building atlas + extras atlas creation (build_extras_atlas), click_to_select_system (ClickSelectParams SystemParam, double-click fountain → Factions tab, building selection stores kind+slot from EntityMap, dead NPC guard via EntityMap.entities, right-click: placing_target mode sets squad.target, default mode commands DirectControl NPCs only with manual_target + MovementIntents.submit(DirectControl) + wakes Resting/GoingToRest NPCs to Idle), box_select_system (drag-rectangle multi-select of player military NPCs via NpcEntry.job.is_military(), sets NpcFlags.direct_control + inserts SquadId via Commands, HashSet for O(1) membership checks, clears SelectedNpc + SelectedBuilding so inspector shows DC group view)
  ...
  src/components.rs     # ECS components for all entities: buildings (EntitySlot, Position, Health, Faction, TownId, Building, FarmReadyMarker, Dead, LastHitBy) + NPC components (Job, Activity, CombatState, Energy, Speed, Home, Personality, CachedStats, BaseAttackType, AttackTimer, ManualTarget, PatrolRoute, WorkPosition, AssignedFarm, CarriedGold, EquippedWeapon/Helmet/Armor, FleeThreshold, LeashRange, WoundedThreshold, NpcFlags, SquadId, Stealer, HasEnergy — all derive Component), TraitKind/TraitInstance for personality system
  ...
  src/ui/
    ...
    game_hud.rs         # Top bar (food + gold + FPS), jukebox overlay (track picker + pause/skip/loop/speed), floating inspector with combat stats/equipment/status/carried loot + clickable faction link (bottom-left) + DC group inspector (unit count/HP/job breakdown/keep-fighting toggle when box-selected) + combat log with faction filter dropdown + clickable ">>" camera-pan on location entries (bottom-right), registry-driven spawner inspector (def.spawner + npc_def for labels, linked NPC state/squad/patrol/position via ECS queries), wall inspector (tier name/HP bar/upgrade button with cost check + deduction), mine assignment UI (slot-based via EntityMap building data), target overlay, squad overlay (numbered target circles hidden during box-select, DirectControl crosshair from ManualTarget::Npc/Building queries), selection overlay (cyan brackets for selected NPC, green brackets for DirectControl NPCs), BuildingInspectorData SystemParam (18 fields incl. ECS queries for Personality/Home/Equipment/CarriedGold/PatrolRoute), expanded Copy Debug Info (NPC: XP/traits/stats/equipment/squad/DirectControl/gold/miner details; Building: faction/per-type details)
  ...
  src/systems/
    ...
    health.rs           # Unified damage_system (NPC + building damage via EntityMap routing — get_instance for buildings, entities for NPCs, SetDamageFlash), unified death_system (mark dead + XP grant + building destruction + NPC cleanup + despawn, DeathResources SystemParam with ECS queries for Home/Personality/AssignedFarm/WorkPosition, hide_npc/hide_building helpers), healing
    behavior.rs         # Unified decision system (ECS queries via DecisionNpcState + NpcDataQueries SystemParam bundles + Commands for optional component insert/remove), arrivals (ECS Home/AssignedFarm queries), squad sync (military NPCs via NpcEntry.job.is_military())
    economy.rs          # Game time, farm growth, mine regen, mining_policy_system (auto-discovery + miner distribution), spawner_respawn_system (iterates EntityMap spawner instances), squad_cleanup_system (NpcEntry.job.is_military(), wave-aware), migration spawn/attach/settle (MigrationResources SystemParam with NpcFlags + Home queries, wipeout → respawn after 4h delay)
