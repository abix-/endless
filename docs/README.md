...
rust/
  ...
  src/systems/
    ...
    economy.rs          # Game time, farm growth (iter_instances_mut with match on Farm/GoldMine, precomputed per-town yield multipliers), mine regen, raider_forage_system (timer-based: accumulates hours per raider town via RaiderState.forage_timers, grants 1 food when reaching raider_forage_hours threshold, 0=disabled), mining_policy_system (auto-discovery + miner distribution), spawner_respawn_system (iterates EntityMap spawner instances), starvation_system (query-first, HP cap clamp for starving NPCs), squad_cleanup_system (query-first recruit pool, wave-aware), farm_visual_system (cadenced every 4th frame), migration spawn/attach/settle (MigrationResources SystemParam with NpcFlags + Home queries, wipeout → respawn after 4h delay)
  ...
  assets/                 # Standard Bevy asset dir (embedded in release builds via bevy_embedded_assets)
    ...
    shaders/
      ...
      npc_render.wgsl       # WGSL render shader (triple vertex path: storage buffer NPC/overlay + instanced + selection brackets with procedural corner fragments at atlas_id=9)
