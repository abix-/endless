...
rust/
  ...
  src/ui/
    ...
    game_hud.rs         # Top bar (food + gold + FPS), jukebox overlay (track picker + pause/skip/loop/speed), floating inspector with combat stats/equipment/status + clickable faction link (bottom-left) + combat log with faction filter dropdown + clickable ">>" camera-pan on location entries (bottom-right), registry-driven spawner inspector (def.spawner + npc_def for labels), mine assignment UI (click-to-assign), target overlay, squad overlay
