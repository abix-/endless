...
rust/
  ...
  src/systems/
    ...
    economy.rs          # Game time, farm growth (iter_instances_mut with match on Farm/GoldMine, precomputed per-town yield multipliers), mine regen, mining_policy_system (auto-discovery + miner distribution), spawner_respawn_system (iterates EntityMap spawner instances), starvation_system (query-first, HP cap clamp for starving NPCs), squad_cleanup_system (query-first recruit pool, wave-aware), farm_visual_system (cadenced every 4th frame), migration spawn/attach/settle (MigrationResources SystemParam with NpcFlags + Home queries, wipeout → respawn after 4h delay)
