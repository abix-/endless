...
rust/
  ...
  src/messages.rs       # Static queues (GpuUpdate), Message types, DirtyWriters SystemParam (dirty signal messages), CombatLogMsg
  ...
  src/systems/
    ...
    combat.rs           # Attack cooldown, targeting, building attack fallback (direct BuildingDamageMsg, visual-only projectile), process_proj_hits routes building hits (entity_idx >= npc_count → BuildingDamageMsg via BuildingEntityMap), building_tower_system (fountain towers via GPU combat_targets readback at npc_count + bld_slot), building_damage_system (entity Health writes via BuildingDeathExtra SystemParam, position/town from BuildingEntityMap.get_instance(), sets BuildingHealState.needs_healing, GPU HP sync via BldSetHealth to BuildingGpuState, skips indestructible: GoldMine + Road)
