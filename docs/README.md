...
rust/
  ...
  src/ui/
    mod.rs              # register_ui(), game startup (+ policy load), cleanup, pause menu (tabbed PauseSettingsTab: Interface/Video/Camera/Controls/Audio/Logs/Debug/SaveGame/LoadGame with save/load/reload/reset actions + quicksave/quickload + named save/load; Controls tab supports key view/rebind/reset), escape/time controls (Playing + Running states), keyboard toggles (defaults: Q=squads, H=help; configurable via key bindings), build ghost preview (Road drag-line with ghost trail), slot indicators, process_destroy_system (resolves exact building slot by kind+town+grid before lethal DamageMsg), apply_ui_scale, apply_interface_text_size (global egui text sizing from UserSettings), test scene systems (bottom panel + target overlay + pause menu in Running state)
    main_menu.rs        # Main menu with sectioned layout (World / Difficulty / Options / Debug Options), Continents-only worldgen (no gen_style selector, always Continents), difficulty presets (Easy/Normal/Hard) drive endless mode + strength + baseline counts, "Per Town (player & AI)" exposes farms/archer homes only (0..10 caps, strip_disabled_home_jobs removes Miner/Fighter/Crossbow from player sliders), AI Builder Towns + AI Raider Towns sliders, tooltips on controls, Play / Load Game / Debug Tests, restart tutorial button
