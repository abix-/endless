...
rust/
  ...
  src/components.rs     # ECS components (NpcIndex, Job, Energy, Health, LastHitBy, BaseAttackType, CachedStats, Activity/CombatState enums, SquadId, CarriedGold, MiningProgress, Archer/Farmer/Miner/Crossbow markers, Migrating, Job::is_patrol_unit/is_military helpers)
  src/constants.rs      # Tuning parameters (grid size, separation, energy rates, TowerStats struct + FOUNTAIN_TOWER const, squad limits, mining, MAX_MINE_OCCUPANCY, 8x8 base build area, WAYPOINT_COVER_RADIUS, MAX_NPC_COUNT=100K, ATLAS_* IDs, SPRITE_CROSSBOW), BUILDING_REGISTRY (single source of truth: BuildingDef with kind/tile/hp/cost/label/spawner/placement/tower_stats), building_def(kind), tileset_index(kind), building_cost(kind), TileSpec, SpawnerDef, SpawnBehavior, PlacementMode, OnPlace
  src/resources.rs      # Bevy resources (SlotAllocator, GameTime, FactionStats, TowerState/TowerKindState, SquadState/SquadOwner, GoldStorage, MineStates, MinerProgressRender, BuildingHpState, MiningPolicy, BuildingSlotMap, BuildMenuContext (selected_build: Option<BuildingKind>, destroy_mode: bool), HelpCatalog, etc.)
  ...
  src/world.rs          # World data structs (GoldMine, MinerHome{assigned_mine}, FarmerHome, ArcherHome, CrossbowHome, Waypoint), world grid, procedural generation (mine placement), tileset builder (build_tileset for terrain array, build_building_atlas for building strip), town grid, building placement/removal, BuildingSpatialGrid (CPU spatial grid for O(1) building lookups, faction-aware), building GPU slot allocation (allocate_building_slot with tileset_idx + tower flag, free_building_slot, allocate_all_building_slots), BuildingKind enum (Fountain/Bed/Waypoint/Farm/Camp/FarmerHome/ArcherHome/Tent/GoldMine/MinerHome/CrossbowHome), Building enum (instances with per-building data), building_tiles() (tile specs from registry), shared helpers: is_alive(), empty_slots(), build_and_pay() (includes dirty flag), place_waypoint_at_world_pos(), register_spawner(), resolve_spawner_npc() (uses SpawnBehavior from registry), destroy_building(), find_nearest_enemy_building(), Building::kind()/spawner_kind()/is_tower()/tileset_index() (all delegate to registry), WorldData::miner_home_at()/gold_mine_at()/building_counts()
  src/ui/
