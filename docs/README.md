...
Bevy ECS (lib.rs build_app)
    ...
    │   ├─ Pause menu (ESC): Resume, categorized Settings (Interface/Camera/Audio/Logs/Debug + Save Game + Load Game), game quicksave/quickload + manual named save/load actions, separate settings-file save/reload/reset controls, Exit to Main Menu — available in both Playing and Running (test scenes)
...
rust/
  ...
  src/settings.rs       # UserSettings persistence (serde JSON save/load, version migration v10, npc_counts BTreeMap<String,usize> with legacy farmers/archers/raiders migration, auto_upgrades, upgrade_expanded (Vec<String> for collapsible branch persistence), autosave_hours, music/sfx volume, music speed, tutorial_completed, log_faction_filter, debug_ai_decisions, interface_text_size, zoom_speed/zoom_min/zoom_max/lod_transition, npc_log_mode (NpcLogMode: All/Faction/SelectedOnly))
  ...
  src/ui/
    ...
    main_menu.rs        # Main menu with sectioned layout (World / Difficulty / Options / Debug Options), Continents-only worldgen, difficulty presets (Easy/Normal/Hard) drive endless mode + strength + baseline counts, "Per Town (player & AI)" exposes farms/mines/farmer homes/archer homes only (0..10 caps), AI Builder Towns + AI Raider Towns sliders, tooltips on controls, Play / Load Game / Debug Tests, restart tutorial button
  ...
  src/systems/
    ...
    health.rs           # Unified damage_system (NPC + building damage via EntityMap routing — get_instance for buildings, entities for NPCs, SetDamageFlash), unified death_system (mark dead + XP grant + building destruction + NPC cleanup + despawn + loot drop MarkVisualDirty, DeathResources SystemParam with ECS queries for Home/Personality/NpcWorkState, hide_npc/hide_building helpers, guards against double-release when work_target == occupied_slot), healing_system (candidate-driven: ActiveHealingSlots resource with cadenced enter-check via npcs_for_town + sustain-check with hysteresis radii, HashMap<i32> faction→zone lookup, emits MarkVisualDirty on healing flag changes)
