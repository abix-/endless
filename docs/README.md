...
rust/
  ...
  src/constants.rs      # Tuning parameters (grid size, separation, energy rates, TurretStats struct + WAYPOINT_TURRET/FOUNTAIN_TURRET consts, squad limits, mining, MAX_MINE_OCCUPANCY, building HP, building costs, 8x8 base build area, WAYPOINT_COVER_RADIUS, MAX_NPC_COUNT=100K, ATLAS_* IDs)
  src/resources.rs      # Bevy resources (SlotAllocator, GameTime, FactionStats, TurretState/TurretKindState, SquadState/SquadOwner, GoldStorage, MineStates, MinerProgressRender, BuildingHpState, MiningPolicy, BuildingSlotMap, HelpCatalog, etc.)
  ...
  src/systems/
    ...
    combat.rs           # Attack cooldown, targeting, building attack fallback, building_turret_system (fountain + waypoint turrets via shared fire_turrets helper), building_damage_system (GPU HP sync via BuildingSlotMap)
