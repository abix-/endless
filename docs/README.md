...
rust/
  ...
  src/ui/
    mod.rs              # register_ui(), game startup (+ policy load), cleanup, shared settings_panel_ui() (renders all settings tabs into a ui region, returns SettingsResponse with reset/save/load flags + paths; callers handle side effects — used by both pause menu and main menu), pause menu (wraps settings_panel_ui with Save/Load tabs + Resume/Exit buttons), escape/time controls (Playing + Running states), keyboard toggles (defaults: Q=squads, H=help; configurable via key bindings), build ghost preview (Road drag-line with ghost trail), slot indicators, process_destroy_system (resolves exact building slot by kind+town+grid before lethal DamageMsg), apply_ui_scale, apply_interface_text_size (global egui text sizing from UserSettings), test scene systems (bottom panel + target overlay + pause menu in Running state)
    main_menu.rs        # Main menu with sectioned layout (World / Difficulty / Options / Debug Options), Continents-only worldgen (no gen_style selector, always Continents), difficulty presets (Easy/Normal/Hard) drive endless mode + strength + baseline counts, "Per Town (player & AI)" exposes farms/archer homes only (0..10 caps, strip_disabled_home_jobs removes Miner/Fighter/Crossbow from player sliders), AI Builder Towns + AI Raider Towns sliders, tooltips on controls, Play / Load Game / Settings / Debug Tests, restart tutorial button; Settings button opens floating window with shared settings_panel_ui (same tabs as pause menu minus Save/Load), side effects applied via pre/post snapshot pattern (video/audio/winit)
