...
rust/
  ...
  src/render.rs         # 2D camera, texture atlases (SpriteAssets with registry-driven external_textures Vec), TilemapChunk spawning (terrain only), TerrainChunk sync, building atlas + extras atlas creation (build_extras_atlas), click_to_select_system (ClickSelectParams SystemParam, double-click fountain → Factions tab, building selection stores kind+slot from EntityMap, dead NPC guard via EntityMap.entities, right-click: placing_target mode sets squad.target, default mode commands DirectControl NPCs only with manual_target + MovementIntents.submit(DirectControl) + wakes Resting/GoingToRest NPCs to Idle), box_select_system (drag-rectangle multi-select of player military NPCs, sets direct_control + squad_id on NpcInstance, HashSet for O(1) membership checks, clears SelectedNpc + SelectedBuilding so inspector shows DC group view)
  ...
  src/components.rs     # ECS components for buildings (EntitySlot, Position, Health, Faction, TownId, Building, FarmReadyMarker, Dead, LastHitBy) + plain data types for NpcInstance fields (Job enum with registry methods, Activity/CombatState enums, Energy, Speed, Home, Personality, CachedStats, BaseAttackType, AttackTimer, ManualTarget enum, PatrolRoute, WorkPosition, AssignedFarm, CarriedGold, EquipLayer, EquippedWeapon/Helmet/Armor, FleeThreshold, LeashRange, WoundedThreshold, TraitKind/TraitInstance) — NPC marker structs removed (Archer/Farmer/Miner/Crossbow/SquadUnit/Stealer/DirectControl/AtDestination/Healing/Starving/Migrating/SquadId all deleted), NPC data types no longer derive Component (state lives in NpcInstance)
  ...
  src/systems/
    spawn.rs            # Spawn system (MessageReader<SpawnNpcMsg>), materialize_npc() shared helper (single source of truth for fresh spawn + save-load, spawns ECS entity with only EntitySlot, builds NpcInstance with all fields, registers in EntityMap), NpcSpawnOverrides, build_patrol_route(&EntityMap)
    ...
    movement.rs         # GPU position readback (EntityMap-only, no ECS query — reads/writes NpcInstance.position + at_destination), arrival detection, resolve_movement_system (sole SetTarget emitter via MovementIntents priority arbitration)
    combat.rs           # Attack cooldown, GPU-unified targeting (NPC + building targets via EntityMap check), process_proj_hits (unified DamageMsg), building_tower_system (fountain towers via GPU combat_targets readback at unified slot)
    health.rs           # Unified damage_system (NPC + building damage via EntityMap routing — get_instance for buildings, entities for NPCs, SetDamageFlash), unified death_system (mark dead + XP grant + building destruction + NPC cleanup + despawn, DeathResources SystemParam, hide_npc/hide_building helpers), healing
    behavior.rs         # Unified decision system (EntityMap-only, no Commands), arrivals, squad sync (all military NPCs via EntityMap)
    economy.rs          # Game time, farm growth, mine regen, mining_policy_system (auto-discovery + miner distribution), spawner_respawn_system (iterates EntityMap spawner instances), squad_cleanup_system (EntityMap iteration, wave-aware), migration spawn/attach/settle (wipeout → respawn after 4h delay)
