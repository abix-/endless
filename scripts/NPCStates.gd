...
@export var verbose_debugging: bool = false
...
# Precomputed movement states for faster checking
var farmer_movement_states = [
	BaseState.MOVING,
	FarmerState.WALKING_TO_FIELD,
	FarmerState.WALKING_HOME
]

var bandit_movement_states = [
	BaseState.MOVING, 
	BanditState.PATROLLING,
	BanditState.CHASING
]
...
func _init(p_owner, p_npc_type):
	...
	# Always initialize state names dictionary - this is required for state name lookup
...
# Helper method to check if current state is a movement state - Now more efficient
func is_movement_state():
	if npc_type == NPCType.FARMER:
		return current_state in farmer_movement_states
	elif npc_type == NPCType.BANDIT:
		return current_state in bandit_movement_states
