...
fn build_death_bench_app() -> App {
    let mut app = build_bench_app();
    spawn_bench_town(&mut app);
    populate_npcs(&mut app, BENCH_TOTAL_NPCS);
    app
}

fn mark_dead_npcs(app: &mut App, death_count: usize) {
    let _ = app
        .world_mut()
        .run_system_once(move |world_mut: &mut World| {
            ...
            let mut q = world_mut
                .query_filtered::<(Entity, &mut Health), (Without<Building>, Without<Dead>)>();
...
fn build_death_pipeline_input(death_count: usize) -> DeathPipelineInput {
    let mut app = build_death_bench_app();
    let victims: Vec<Entity> = {
        let world = app.world_mut();
        let mut q = world.query_filtered::<Entity, (Without<Building>, Without<Dead>)>();
        q.iter(world).take(death_count).collect()
    };
    DeathPipelineInput {
        app,
        victims: Arc::from(victims),
    }
}

fn bench_death_system_idle(c: &mut Criterion) {
    let mut group = c.benchmark_group("death_system_idle");
    group.sample_size(20);
    group.bench_function("50000", |b| {
        b.iter_batched_ref(
            build_death_bench_app,
            |app| {
...
fn bench_death_system(c: &mut Criterion) {
    ...
    group.sample_size(20);
    // Victims are pre-marked Dead during setup so the timed section only covers
    // death_system cleanup work, not detection, respawn, or benchmark setup.
        ...
        group.bench_with_input(
            ...
            |b, &death_count| {
                b.iter_batched_ref(
                    || {
                        let mut app = build_death_bench_app();
                        mark_dead_npcs(&mut app, death_count);
                        app
                    },
                    |app| {
                        let _ = app.world_mut().run_system_once(death_system);
...
fn bench_death_pipeline(c: &mut Criterion) {
    let mut group = c.benchmark_group("death_pipeline");
    group.sample_size(20);
    // Measure the live combat-side handoff: damage_system marks victims Dead,
    // then death_system processes them. Respawn is benchmarked separately.
    for &death_count in DEATH_COUNTS {
        group.bench_with_input(
            BenchmarkId::from_parameter(death_count),
            &death_count,
            |b, &death_count| {
                b.iter_batched_ref(
                    || build_death_pipeline_input(death_count),
                    |input| {
                        let victims = Arc::clone(&input.victims);
                        let _ = input.app.world_mut().run_system_once(
                            move |mut writer: MessageWriter<DamageMsg>| {
                                for &entity in victims.iter() {
                                    writer.write(DamageMsg {
                                        target: entity,
                                        amount: 200.0,
                                        attacker: -1,
                        ...
                        let _ = input.app.world_mut().run_system_once(damage_system);
                        input.app.world_mut().flush();
                        let _ = input.app.world_mut().run_system_once(death_system);
                        input.app.world_mut().flush();
                    },
                    BatchSize::PerIteration,
                );
