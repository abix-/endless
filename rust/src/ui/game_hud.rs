...
use crate::ui::tipped;
use crate::world::{WorldData, WorldGrid, BuildingKind, is_alive};
        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    let total_spawners: usize = entity_map.iter_instances()
...
pub fn bottom_panel_system(
    ...
    let dc_count = bld_data.entity_map.iter_npcs().filter(|n| !n.dead && bld_data.npc_flags_q.get(n.entity).is_ok_and(|f| f.direct_control)).count();
        ...
        egui::Window::new("Inspector")
            ...
            .show(ctx, |ui| {
                if has_npc && has_building {
                    ...
                    let bld_label = selected_building_info(&bld_data.selected_building, &bld_data.grid, &bld_data.entity_map)
                ...
                if show_building {
                    let selected_info = selected_building_info(&bld_data.selected_building, &bld_data.grid, &bld_data.entity_map);
...
fn dc_group_inspector(
    ...
    for npc in entity_map.iter_npcs() {
        if npc.dead || !npc_flags_q.get(npc.entity).is_ok_and(|f| f.direct_control) { continue; }
        count += 1;
        total_hp += health_q.get(npc.entity).map(|h| h.0).unwrap_or(0.0);
        total_max_hp += cached_stats_q.get(npc.entity).map(|s| s.max_health).unwrap_or(100.0);
...
fn inspector_content(
    ...
    if !show_npc {
        ...
        if dc_count > 0 {
            dc_group_inspector(ui, &bld_data.entity_map, &bld_data.npc_flags_q, squad_state, &bld_data.npc_health_q, &bld_data.cached_stats_q);
            return;
        }
    ...
    let (hp, max_hp, energy, cached_stats) = if let Some(npc) = bld_data.entity_map.get_npc(idx) {
        let hp = bld_data.npc_health_q.get(npc.entity).map(|h| h.0).unwrap_or(0.0);
        let cs = bld_data.cached_stats_q.get(npc.entity).cloned().ok();
        let max_hp = cs.as_ref().map(|s| s.max_health).unwrap_or(100.0f32);
        let energy = bld_data.energy_q.get(npc.entity).map(|e| e.0).unwrap_or(0.0);
        (hp, max_hp, energy, cs)
    } else {
    ...
    if let Some(npc) = bld_data.entity_map.get_npc(idx) {
        let atk_type = bld_data.cached_stats_q.get(npc.entity).map(|_| ()).ok()
    ...
    // DirectControl toggle (separate borrow scope)
    {
        ...
        let is_dc = entity.and_then(|e| bld_data.npc_flags_q.get(e).ok()).is_some_and(|f| f.direct_control);
        ui.horizontal(|ui| {
    ...
    if let Some(npc) = bld_data.entity_map.get_npc(idx) {
        ...
        let combat_name = bld_data.combat_state_q.get(npc.entity).map(|cs| cs.name()).unwrap_or("");
    ...
    if meta.job == 4 {
        if let Some(hp) = home_pos {
            let mh_slot = bld_data.entity_map.find_by_position(hp)
                .filter(|i| i.kind == BuildingKind::MinerHome)
                .map(|i| i.slot);
            if let Some(mh_slot) = mh_slot {
                ui.separator();
                mine_assignment_ui(ui, world_data, &mut bld_data.entity_map, mh_slot, hp, dirty_writers, ui_state);
                // Show mine productivity when actively mining
                if is_mining_at_mine {
                    let mine_pos = bld_data.entity_map.get_instance(mh_slot).and_then(|i| i.assigned_mine);
                    if let Some(mine_pos) = mine_pos {
                        let occupants = bld_data.entity_map.slot_at_position(mine_pos).map(|s| bld_data.entity_map.occupant_count(s)).unwrap_or(0);
                        if occupants > 0 {
    ...
    if settings.debug_coordinates {
        ...
        if ui.button("Copy Debug Info").clicked() {
            ...
            if meta.job == 4 {
                if let Some(hp) = home_pos {
                    if let Some(inst) = bld_data.entity_map.find_by_position(hp).filter(|i| i.kind == BuildingKind::MinerHome) {
...
fn mine_assignment_ui(
    ...
    let (assigned, manual) = entity_map.get_instance(mh_slot)
    ...
    ui.horizontal(|ui| {
        if ui.button("Set Mine").clicked() {
            if let Some(inst) = entity_map.get_instance_mut(mh_slot) { inst.manual_mine = true; }
...
fn building_inspector_content(
    ...
    match kind {
        BuildingKind::Farm => {
            if let Some(inst) = bld.entity_map.find_farm_at(world_pos) {
                ...
                let occupants = bld.entity_map.slot_at_position(world_pos).map(|s| bld.entity_map.occupant_count(s)).unwrap_or(0);
        ...
        BuildingKind::Wall => {
            ...
            if let Some(wall_inst) = bld.entity_map.find_by_position(world_pos) {
                ...
                {
                    let hp = bld.entity_map.entities.get(&wall_inst.slot)
        ...
        _ => {
            if let Some(spawner) = def.spawner {
                ...
                if def.kind == BuildingKind::MinerHome {
                    ui.separator();
                    let mh_slot = bld.entity_map.find_by_position(world_pos)
                        .filter(|i| i.kind == BuildingKind::MinerHome)
                        .map(|i| i.slot);
                    if let Some(mh_slot) = mh_slot {
                        mine_assignment_ui(ui, world_data, &mut bld.entity_map, mh_slot, world_pos, dirty_writers, ui_state);
                    }
                }
            }
        }
    ...
    if settings.debug_coordinates {
        ...
        let hp = bld.entity_map.find_by_position(world_pos)
            ...
            match kind {
                ...
                BuildingKind::GoldMine => {
                    ...
                    if let Some(inst) = bld.entity_map.find_mine_at(world_pos) {
                        ...
                        if inst.occupants > 0 {
                            let mult = crate::constants::mine_productivity_mult(inst.occupants as i32);
                            info.push_str(&format!("Miners: {} ({:.0}% speed)\n", inst.occupants, mult * 100.0));
                        }
                    }
                }
                _ => {}
...
pub fn selection_overlay_system(
    ...
    let dc_slots: Vec<usize> = entity_map.iter_npcs().filter(|n| !n.dead && npc_flags_q.get(n.entity).is_ok_and(|f| f.direct_control)).map(|n| n.slot).collect();
...
pub fn squad_overlay_system(
    ...
    let dc_targets: Vec<ManualTarget> = entity_map.iter_npcs()
        .filter(|n| !n.dead && npc_flags_q.get(n.entity).is_ok_and(|f| f.direct_control))
        .filter_map(|n| manual_target_q.get(n.entity).ok().cloned())
        .collect();
