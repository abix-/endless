...
pub fn tick(
    ...
    let energy = entity_map
        .iter_npcs()
        .find(|n| !n.dead && n.job == Job::Archer)
        .and_then(|n| energy_q.get(n.entity).ok())
        .map(|e| e.0)
        .unwrap_or(100.0);
    let on_duty = entity_map
        .iter_npcs()
        .filter(|n| {
            !n.dead
                && n.job == Job::Archer
    ...
    match test.phase {
        ...
        2 => {
            test.phase_name = format!(
                "patrolling={} on_duty={} e={:.0}",
                patrolling, on_duty, energy
            );
            if patrolling > 0 {
                test.pass_phase(elapsed, format!("Patrolling (energy={:.0})", energy));
            } else if elapsed > 15.0 {
                let ticks = entity_map
                    .iter_npcs()
                    .find_map(|n| {
                        activity_q.get(n.entity).ok().and_then(|a| {
                            if a.kind == ActivityKind::Patrol && a.phase == ActivityPhase::Holding {
        ...
        3 => {
            test.phase_name = format!(
                "on_duty={} patrolling={} e={:.0}",
                on_duty, patrolling, energy
            );
            if on_duty > 0 {
        ...
        4 => {
            ...
            if resting > 0 || going_rest > 0 {
                ...
                test.fail_phase(
                    elapsed,
                    format!("energy={:.0} never reached hungry", energy),
                );
            }
        }
        ...
        5 => {
            test.phase_name = format!(
                "e={:.0} on_duty={} patrolling={} resting={}",
                energy, on_duty, patrolling, resting
            );
