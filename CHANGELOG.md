...
- **gold mines** — wilderness resource nodes placed between towns during world gen; farmers walk out to mine gold based on `mining_pct` policy slider (0-100%); miners claim occupancy, extract `MINE_EXTRACT_PER_CYCLE` gold when tired, carry it home via `Activity::Returning { gold }` for proximity delivery to `GoldStorage`; mines regenerate `MINE_REGEN_RATE` gold/hour when unoccupied (capped at `MINE_MAX_GOLD`); mines are unowned — any faction's farmers can use any mine; `Building::GoldMine` variant with `TileSpec::Single(43, 11)` sprite; HUD top bar shows gold count; mine inspector shows gold amount + progress bar + miner count; main menu slider for gold mines per town (0-10); AI personalities set mining allocation (Aggressive 20%, Balanced 40%, Economic 60%); `MineStates` + `GoldStorage` resources; `mine_regen_system` in Step::Behavior; `WorldGenConfig.gold_mines_per_town` persisted in settings
